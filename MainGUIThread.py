...
class LoadOpticalTrap(QObject):
    global win, camObject, camObject2, energy3000, time_unit, time_unit2
    new_cycle = pyqtSignal(int)
    update_ccd_pic = pyqtSignal(np.ndarray)
    update_profile_plot = pyqtSignal(np.ndarray, bool, int)
    finished = pyqtSignal()    
        
    def run(self):        
        win.stopVideo()
        if (win.dipoleTransferROIBackgnd.isChecked()):
            repetitions = win.averageSample.value()
        else:
            repetitions = 1
        if win.loadDipoleTrapAbsorpImag.isChecked():
            readout_time = 148000
        else:
            readout_time = 90000
        ...
        try:
            ...
            # switch off pwr supply
            win.adw.Set_Par(3, win.adw.Get_Par(3) & 0b0111111111111) 
            # sequence, local  
            energy3000.setModes(3,0) 
            # switch on pwr supply
            win.adw.Set_Par(3, win.adw.Get_Par(3) | 0b1000000000000) 
            ...
            win.adwPro2.Stop_Process(2)
            win.adw.Stop_Process(2)
            # set ending condition to false
            win.adw.Set_Par(78,0)
            win.adwPro2.Set_Par(78,0)
            # set sequences into waiting loop
            win.adw.Set_Par(80,0)
            win.adwPro2.Set_Par(80,0)
            # since process 1 on ADWIN Gold needs an external trigger, start this process first
            win.adw.Start_Process(1)
            # starts ADwin Pro II process 1, which sends event signals for process 1 on ADwin GOLD
            win.adwPro2.Start_Process(1)
            ###########################################################################
            
            # delay time of uniblitz shutter
            win.adwPro2.Set_Par(60, int(math.ceil(win.uniblitzDelay.value()/time_unit2)))
            
            # write important timings into ADwin variables
            # shutter delay of 10 ms
            win.adw.Set_Par(12, int(math.ceil(win.ovenShutterDelay.value()/time_unit)))
            win.adwPro2.Set_Par(12, int(math.ceil(win.ovenShutterDelay.value()/time_unit2)))
            # time for loading @ load detuning and intensities
            win.adw.Set_Par(13, int(math.ceil(win.loadingTime3.value()/time_unit)))
            win.adwPro2.Set_Par(13, int(math.ceil(win.loadingTime3.value()/time_unit2)))
            # trigger delay time
            win.adw.Set_Par(53, int(math.ceil(win.trigDelayPxUsb.value()/10/time_unit)))
            win.adwPro2.Set_Par(53, int(math.ceil(win.trigDelayPxUsb.value()/10/time_unit2)))
            # exposure time
            if (win.loadDipoleTrapAbsorpImag.isChecked()):
                win.adw.Set_Par(25, int(math.ceil(win.absorpImgExpTime.value()/time_unit)))
                win.adwPro2.Set_Par(25, int(math.ceil(win.absorpImgExpTime.value()/time_unit2)))
            else:
                win.adw.Set_Par(25, int(math.ceil(win.exposure_time.value()/time_unit)))
                win.adwPro2.Set_Par(25, int(math.ceil(win.exposure_time.value()/time_unit2)))
            # exposure time + read out time            
            win.adw.Set_Par(15, int(math.ceil(readout_time/time_unit)))
            win.adwPro2.Set_Par(15, int(math.ceil(readout_time/time_unit2))) 
            # optical trap time
            # close oven shutter during optical trap time or not
            win.adw.Set_Par(23, int(win.close_oven_shutter.isChecked()))
            win.adw.Set_Par(22, int(math.ceil(win.opticaltraptime.value()/time_unit)))
            win.adwPro2.Set_Par(22, int(math.ceil(win.opticaltraptime.value()/time_unit2)))
            # ionization delay
            win.adw.Set_Par(69, int(math.ceil(win.ionizationDelay.value()/time_unit)))
                        

            # setting initial and target values
            # for beat offset value
            win.adw.Set_FPar(11, win.vcoBeatOffset.value())
            
            # for cooler intensity
            win.adw.Set_FPar(17, win.vcaCooler.value())
            # for repumper intensity
            win.adw.Set_FPar(18, win.vcaRepumper.value())            
            # for dipole laser power
            win.adw.Set_FPar(19, win.fiberLaserAnalogIn.value()) # initial dipole power       
            win.adw.Set_FPar(23, win.maxIRPower.value()) # maximum IR power / trap depth
            # for rf driver power
            win.adw.Set_FPar(20, win.RFdriveramp.value()) # initial rf power
            win.adw.Set_FPar(24,5)

            #for pushing beam
            win.adwPro2.Set_Par(59, int(win.pushBeam.isChecked()))
            win.adwPro2.Set_Par(62, int(math.ceil(win.pushTime.value()/time_unit2)))
            win.adwPro2.Set_Par(46, int(math.ceil(win.pushBeamDelay.value()/time_unit2)))
            
            # Kniel delays
            win.adw.Set_Par(39, int(math.ceil(win.knielDelayTime.value()/time_unit)))
            win.adwPro2.Set_Par(39, int(math.ceil(win.knielDelayTime.value()/time_unit2)))
            win.adw.Set_Par(41, int(math.ceil(win.knielTrigDelay.value()/time_unit)))
            # recooling time
            win.adw.Set_Par(48, int(math.ceil(win.recoolTime.value()/time_unit)))
            win.adwPro2.Set_Par(48, int(math.ceil(win.recoolTime.value()/time_unit2)))
            
            if (win.evapCool.isChecked()):
                # collision rate in MHz
                coll_rate = 5.821E-9*win.initialAtomNumber.value()*win.eta.value()
                eta_prime = win.eta.value()+(win.eta.value()-5)/(win.eta.value()-4)
                # evaporation time constant in mus
                tau_u = 1.5/((win.eta.value()-4)*(eta_prime-6)*np.exp((-1)*win.eta.value())*coll_rate)
                exponent = 2*(eta_prime-3)/(eta_prime-6) 
                evapTime = tau_u*(1.0-(win.ctrlVoltFinal.value()/10)**(1.0/exponent))
                win.adw.Set_FPar(66, exponent)
                win.adw.Set_Par(66, int(math.ceil(evapTime/time_unit)))
                win.adwPro2.Set_Par(66, int(math.ceil(evapTime/time_unit2)))
                win.adw.Set_Par(67, int(math.ceil(tau_u/time_unit)))
                win.adwPro2.Set_Par(67, int(math.ceil(tau_u/time_unit2)))
            else:        
                win.adw.Set_Par(66, 0)
            
            # for ionization of atoms out of dipole trap
            win.adw.Set_Par(44, int(win.openfsShut.isChecked()))
            # for data acquisition after magnetic field has been switched off
            win.adw.Set_Par(51, int(win.daqEnable_2.isChecked()))
            win.adwPro2.Set_Par(51, int(win.daqEnable_2.isChecked()))
            win.adwPro2.Set_Par(70, int(math.ceil(win.motxyshutdelay.value()/time_unit)))
            # for switching on the PA beam during the sequence
            win.adwPro2.Set_Par(54, int(win.PAbeam.isChecked()))
            win.adw.Set_Par(54, int(win.PAbeam.isChecked()))
            
                    
                
            # delay for switching off the current of the MOT coils
            win.adw.Set_Par(45, int(math.ceil(win.currentOffDelay.value()/time_unit)))
            win.adwPro2.Set_Par(45, int(math.ceil(win.currentOffDelay.value()/time_unit2)))
            # response delay of femto shutter
            win.adw.Set_Par(56, int(math.ceil(win.femtoShutterDelay.value()/time_unit)))
            win.adwPro2.Set_Par(56, int(math.ceil(win.femtoShutterDelay.value()/time_unit2)))
                            
            win.adw.Set_Par(36, int(win.dipoleTransferROIBackgnd.isChecked()))
            win.adwPro2.Set_Par(36, int(win.dipoleTransferROIBackgnd.isChecked()))
            win.adw.Set_Par(37, int(win.loadDipoleTrapAbsorpImag.isChecked()))
            win.adwPro2.Set_Par(37, int(win.loadDipoleTrapAbsorpImag.isChecked()))

            win.adwPro2.Set_Par(52, int(win.PASync.isChecked()))
            win.adwPro2.Set_Par(33, int(win.IRSync.isChecked()))
                        
            # number of times sequence will be repeated
            win.adw.Set_Par(11, repetitions)
            win.adwPro2.Set_Par(11, repetitions)
        ...
        if(not(win.dipoleTransferROIBackgnd.isChecked())):
            ...
            exp_params.write("PROCESSDELAY = " + str(win.adw.Get_Processdelay(1)) + "\n")
            exp_params.write("All ADwin times in units of PROCESSDELAY!\n")
            exp_params.write("Loading time: " + str(win.loadingTime3.value())+ "\n")
            exp_params.write("Load Detuning [V]: " + str(win.vcoBeatOffset.value())+"\n")
            exp_params.write("Detuning [units of Gamma]: " + str(win.detuningFromVoltage(win.vcoBeatOffset.value())/6.0)+"\n")
            exp_params.write("Target detuning [V]: " + str(win.targetDetuning_2.value()) + "\n")
            exp_params.write("Target detuning [units of Gamma]: " + str(win.detuningFromVoltage(win.targetDetuning_2.value())/6.0)+"\n")
            exp_params.write("Frequency ramp rate [µs/0.007 V]: " + str(win.rampingSpeed_2.value())+"\n")
            exp_params.write("VCA Target Cooling Power [V]: " + str(win.vcaCoolerTarget_2.value())+"\n")
            exp_params.write("VCA Target Repumping Power [V]: " + str(win.vcaRepumperTarget_2.value()) + "\n")
            exp_params.write("Molasse cooling time [µs]: " + str(win.molasseCoolTime_2.value()) + "\n")
            exp_params.write("Optical pump time [µs]: " + str(win.optPumpTime.value()) + "\n")
            exp_params.write("Optical trapping time [µs]: " + str(win.opticaltraptime.value()) + "\n")
            exp_params.write("Exposure time [mus]: " + str(win.exposure_time.value())+"\n")
            exp_params.write("Imaging detuning [V]: " + str(win.imagDetuning.value()) + "\n")
            exp_params.close()

        plot_size = 10

        if (win.doPAFreqScan.isChecked()):
            win.xdata = np.arange(win.scanLaserDict[win.scanLaser.currentIndex()].value(), win.PA_SPmax.value(), win.PA_SPstep.value())
            win.ydata = np.zeros(np.shape(win.xdata))
            k = 0
        elif(not(win.dipoleTransferROIBackgnd.isChecked())):
            win.xdata = np.zeros(plot_size)
            win.ydata = np.zeros(plot_size)
                    
        
        sum_img = np.zeros(win.ROIBackground.shape)
        # reinitialize the Background
        if (win.dipoleTransferROIBackgnd.isChecked()):
            win.Background_IRScatt = np.zeros(np.shape(camObject.pic))
        elif not(hasattr(win,'Background_IRScatt')):
            try:
                win.Background_IRScatt = np.load("IRScatt"+str(win.exposure_time.value())+"mus.npy")
            except IOError:
        ...
        if (win.loadDipoleTrapAbsorpImag.isChecked()):
            ...
            camObject2.exposure_time(win.absorpImgExpTime.value(),1, True)
            # arm camera again
            ...
            max_triggers = 2
            if hasattr(win,'drkCnts'):
                Ibg = win.drkCnts*win.absorpImgExpTime.value()
            else:
                Ibg = scipy.ndimage.imread("AI_dark_cnts.png")*win.absorpImgExpTime.value()
            Iabs, Iref = 0,0
                                
        doAbsImg = win.loadDipoleTrapAbsorpImag.isChecked()
        ...
        win.startAsyncMode(repetitions, win.exposure_time.value())
        camObject.waitForTrigger()
        ####################################################
               
        win.noUserInterrupt = True
        win.PAFreqInitArray = False
        ...
        logging = True
        j = 0
        win.movingAvgIndex = 1
        
        if (win.PAFreqInitArray):
            win.PAFreqInitArray = False
            win.xdata = np.arange(win.scanLaserDict[win.scanLaser.currentIndex()].value(), win.PA_SPmax.value(), win.PA_SPstep.value())
            print "win.xdata.size = ", win.xdata.size, "\n"
            k = 0
            win.ydata = np.zeros(np.shape(win.xdata))
        
        while (j < repetitions and win.noUserInterrupt):
            try:
                # log specific analog output voltages during sequence
                if (logging):
                    win.adw.Set_Par(24,0)
                    win.adw.Set_Par(77,1)
                    logging = False
                # beam powers for pulsed cooler and repumper pulses
                win.adw.Set_FPar(42, win.coolerPulsed.value())
                win.adw.Set_FPar(43, win.repumpPulsed.value())
                # imaging detuning and beam powers
                win.adw.Set_FPar(51, win.imagDetuning.value())
                win.adw.Set_FPar(40, win.coolerImag.value())
                win.adw.Set_FPar(41, win.repumpImag.value())
                # imaging beam intensity
                win.adw.Set_FPar(31, win.imagMod.value())                
                # for pump time
                win.adw.Set_Par(42, int(math.ceil(win.optPumpTime.value()/time_unit)))
                win.adwPro2.Set_Par(42, int(math.ceil(win.optPumpTime.value()/time_unit2)))
                # set flight time
                win.adw.Set_Par(14, int(math.floor(win.optTrapFlightTime.value()/time_unit)))
                win.adwPro2.Set_Par(14, int(math.floor(win.optTrapFlightTime.value()/time_unit2)))
                win.adw.Set_FPar(13, win.targetDetuning_2.value())
                # variables for the ramps
                ramptime = int(math.ceil(abs((win.vcoBeatOffset.value()-win.targetDetuning_2.value())/0.007*win.rampingSpeed_2.value()/time_unit)))
                ramptime2 = int(math.ceil(abs((win.vcoBeatOffset.value()-win.targetDetuning_2.value())/0.007*win.rampingSpeed_2.value()/time_unit2)))
                voltstep = 0.007/win.rampingSpeed_2.value()*time_unit
                # total ramp time for the frequency ramp
                win.adw.Set_Par(16, ramptime)
                win.adwPro2.Set_Par(16, ramptime2)
                # volt step for the frequency ramp
                win.adw.Set_FPar(25, voltstep)
                # backramp
                backramptime = int(math.ceil(abs((win.imagDetuning.value()-win.targetDetuning_2.value())/(0.007*time_unit))))
                backramptime2 = int(math.ceil(abs((win.imagDetuning.value()-win.targetDetuning_2.value())/(0.007*time_unit2))))
                backvoltstep = 0.007*time_unit
                # total ramp time for ramping frequency background
                win.adw.Set_Par(50, backramptime)
                win.adwPro2.Set_Par(50, backramptime2)
                # volt step for back frequency ramp
                win.adw.Set_FPar(50, backvoltstep)
                # low intensities for MOT beams near resonance
                win.adw.Set_FPar(15, win.vcaCoolerTarget_2.value())
                win.adw.Set_FPar(16, win.vcaRepumperTarget_2.value())
                # cooling time at final detuning
                win.adw.Set_Par(30, int(math.ceil(win.molasseCoolTime_2.value()/time_unit)))
                win.adwPro2.Set_Par(30, int(math.ceil(win.molasseCoolTime_2.value()/time_unit2)))
                # pushing beam time
                win.adwPro2.Set_Par(62, int(math.ceil(win.pushTime.value()/time_unit2)))
                                
                if (win.PAbeam.isChecked()):
                    win.adw.Set_Par(29, int(math.ceil(win.PAbeamExp.value()/time_unit)))
                    win.adwPro2.Set_Par(29, int(math.ceil(win.PAbeamExp.value()/time_unit2)))
                else:
                    win.adw.Set_Par(29, 0)
                    win.adwPro2.Set_Par(29, 0)
                    
                # volt step for cooler intensity ramp
                if ramptime == 0:
                    win.adw.Set_FPar(28, 0)
                else:
                    win.adw.Set_FPar(28, abs((win.vcaCooler.value()-win.vcaCoolerTarget_2.value())/ramptime))
                # volt step for repumper intensity ramp
                if ramptime == 0:
                    win.adw.Set_FPar(29, 0)
                else:
                    win.adw.Set_FPar(29, (win.vcaRepumper.value()-win.vcaRepumperTarget_2.value())/ramptime)        
                # initialize 
                win.adw.Set_Par(79, 1)
                win.adwPro2.Set_Par(79,1)
                # start case 4, but start it on ADWin Pro II first, because the sequence already runs and is triggering.
                win.adwPro2.Set_Par(80,4)
                win.adw.Set_Par(80, 4)                
            except ADwinError, e:
                print '***', e
            i = 0
            while(i < max_triggers and win.noUserInterrupt):                     
                ...
                if doAbsImg:
                    if(camObject2.waitForBuffer()):
                        ...
                        if (i==1):
                            ...
                            OD *= (-1)                
                            win.roi.setState(camObject2.ROIState, False)
                            ROI_OD = np.array(win.roi.getArrayRegion(OD, win.img))
                            if (win.averageROIProfile.isChecked()):
                                fraction = float(win.movingAvgIndex-1)/(win.movingAvgIndex)
                                print "Index: ", win.movingAvgIndex, "\n"
                                print "Fraction: ", fraction, "\n"
                                ROI_profile = ROI_profile*fraction + ROI_OD.sum(axis=win.roiProfilePlot.currentIndex())/float(win.movingAvgIndex)
                                win.movingAvgIndex +=1
                                print "np.sum(ROI_profile) = ", np.sum(ROI_profile)
                            else:
                                ROI_profile = ROI_OD.sum(axis=win.roiProfilePlot.currentIndex())                            
          
                            self.update_ccd_pic.emit(ODplot)
                            self.update_profile_plot.emit(ROI_profile, True, 1)
                                                        
                            win.xdata = np.roll(win.xdata, -1)
                            win.ydata = np.roll(win.ydata,-1)
                            win.xdata[-1] = win.xdata[-2] + 1
                            win.ydata[-1] = np.max(ROI_OD.sum(axis=win.roiProfilePlot.currentIndex()))
                            win.curve.setData(win.xdata, win.ydata) 
                    ...
                    if camObject.waitForBuffer(0):
                        ...
                        i += 1
                        win.roi.setState(camObject.ROIState, False)
                        if(win.dipoleTransferROIBackgnd.isChecked()):
                            self.update_ccd_pic.emit(camObject.image[j])
                            ROI_profile = np.sum(camObject.image[j],axis=win.roiProfilePlot.currentIndex())
                            # show cloud profile
                            self.update_profile_plot.emit(ROI_profile, True, 0)
                        else:
                            # substract background counts from taken image
                            woBackGnd = np.array(camObject.image[j]-win.Background_IRScatt)
                            # show taken image wo background
                            self.update_ccd_pic.emit(woBackGnd)                     
                                                        
                            # cloud profile in ROI without background
                            if (win.averageROIProfile.isChecked()):
                                fraction = float(win.movingAvgIndex-1.0)/(win.movingAvgIndex)
                                print "Index: ", win.movingAvgIndex, "\n"
                                print "Fraction: ", fraction, "\n"
                                if(win.movingAvgIndex ==1 ):
                                    ROI_woBackGnd = win.roi.getArrayRegion(woBackGnd, win.img)
                                else:
                                    ROI_woBackGnd = ROI_woBackGnd*fraction+win.roi.getArrayRegion(woBackGnd, win.img)/float(win.movingAvgIndex)
                                win.movingAvgIndex +=1
                            else:
                                win.movingAvgIndex = 1
                                ROI_woBackGnd = win.roi.getArrayRegion(woBackGnd, win.img)
                                ROI2_woBackGnd = win.roi2.getArrayRegion(woBackGnd, win.img)

                            ROI_profile = np.sum(ROI_woBackGnd,axis=win.roiProfilePlot.currentIndex())
                            maxpixelcnt = np.max(ROI_woBackGnd)
                            meancnt = np.mean(ROI2_woBackGnd)
                                    
                            print "Maximum count in ROI (one pixel): ", maxpixelcnt, "\n"
                            # calculate averaged 1-sigma peak density:
                            hist, edges = np.histogram(ROI_woBackGnd.flatten(), bins=50)
                            pos = 0.5*(edges[:-1]+edges[1:])
                            prob = hist.astype('float')/np.sum(hist)
                            mu = np.sum(pos*prob)
                            sigma = np.sqrt(np.sum(prob*pos**2)-mu**2)
                            mask = (pos>=(mu+2*sigma))
                            sigmapeak = np.sum(prob[mask]*pos[mask])/np.sum(prob[mask])
                            if (win.plotROIHistogram.isChecked()):
                                if not(hasattr(win, 'ROIHistogramPlot')):
                                    win.ROIHistogramPlot = pg.plot()
                                win.ROIHistogramPlot.plot(edges, hist, stepMode=True,fillLevel=0, brush=(0,0,255,150), clear=True)
                            print "Averaged maximum count in ROI: ", sigmapeak, "\n"          
                        
                            
                            # show cloud profile
                            self.update_profile_plot.emit(ROI_profile, True, 0)
                            
                            if win.doFit.isChecked():
                                ...
                                win.params = fitGaussianProfile(xvalues, ROI_profile, yfit)
                                linOffset = win.params[0][3]+win.params[0][4]*xvalues
                                areaUnderGaussian = np.trapz(yfit,xvalues)-np.trapz(linOffset, xvalues)
                                print("Area under Gaussian: ", areaUnderGaussian)
                                print("Sigma from Gaussian Fit: ", np.sqrt(win.params[0][1]))
                                print("Total atom number in ROI [*10^7]: ", 4*np.pi/4.37*1E-4/(0.226*37*win.exposure_time.value())*areaUnderGaussian)
                                print("Peak density in 10^11 /cm^3 from one pixel in ROI 1: ", 4*np.pi/4.37*1E-8/(0.226*37*win.exposure_time.value())*maxpixelcnt/(0.000645**2*2*np.sqrt(win.params[0][1])/26.316/10))
                                print("Mean density in 10^11 /cm^3 from mean counts in ROI 2: ", 4*np.pi/4.37*1E-8/(0.226*37*win.exposure_time.value())*meancnt/(0.000645**2*2*np.sqrt(win.params[0][1])/26.316/10))
                                print("1 sigma peak density in 10^11 /cm^3 averaged from ROI: ", 4*np.pi/4.37*1E-8/(0.226*37*win.exposure_time.value())*sigmapeak/(0.000645**2*2*np.sqrt(win.params[0][1])/26.316/10))

                                # show fitted profile
                                self.update_profile_plot.emit(yfit, False, 2)     

                        if (win.doPAFreqScan.isChecked() and (k < win.xdata.size)):
                            print "SP = ", win.xdata[k], "\n"                           

                            if win.doFit.isChecked():
                                win.ydata[k] = areaUnderGaussian
                            else:
                                if (win.plotOptions.currentIndex() == 0):
                                    win.ydata[k] = np.sum(ROI_profile)
                                    print "np.sum(ROI_profile) = ", win.ydata[k]
                                if win.plotOptions.currentIndex() == 1:
                                    win.ydata[k] = np.max(ROI_profile)
                                    print "np.max(ROI_profile) = ", win.ydata[k]
                                if (win.plotOptions.currentIndex() == 3):
                                    xvalues = np.arange(0, np.shape(ROI_profile)[0])
                                    offset = (ROI_profile[-1]-ROI_profile[0])*xvalues/xvalues[-1]+ROI_profile[0]
                                    self.update_profile_plot.emit(offset, False, 2)
                                    win.ydata[k] = np.trapz(ROI_profile - offset, xvalues)
                                    print "Area under curve in counts = ", win.ydata[k]
                                if (win.plotOptions.currentIndex()== 4):
                                    win.ydata[k] = maxpixelcnt
                                    print "Count in pixel with max count = ", win.ydata[k]
                            k += 1            
                            if (k >= win.xdata.size):
                                win.adwPro2.Set_Par(78,1)
                                win.adw.Set_Par(78, 1)                                
                                win.noUserInterrupt = False
                            else:
                                win.scanLaserDict[win.scanLaser.currentIndex()].setValue(win.xdata[k])
                            
                                                      
                        if (not(win.doPAFreqScan.isChecked()) and not(win.dipoleTransferROIBackgnd.isChecked())):
                            ...
                            if win.doFit.isChecked():
                                ...
                                win.ydata[-1] = areaUnderGaussian
                            else:
                                if (win.plotOptions.currentIndex() == 0):
                                    win.ydata[-1] = np.sum(ROI_profile)
                                    win.ROIMean = win.ROIMean*(runningindex-1)/runningindex + win.ydata[-1]/runningindex
                                    runningindex += 1 
                                    print "ROI sum: ", win.ROIMean
                                if (win.plotOptions.currentIndex() == 1):
                                    win.ydata[-1] = np.max(ROI_profile)
                                if (win.plotOptions.currentIndex() == 3):
                                    xvalues = np.arange(0, np.shape(ROI_profile)[0])
                                    offset = (ROI_profile[-1]-ROI_profile[0])*xvalues/xvalues[-1]+ROI_profile[0]
                                    self.update_profile_plot.emit(offset, False, 2)
                                    win.ydata[-1] = np.trapz(ROI_profile - offset, xvalues)
                                    print "Area under curve in counts = ", win.ydata[-1]
                                if (win.plotOptions.currentIndex()==4):
                                    win.ydata[-1] = maxpixelcnt
                                    print "Count in pixel with max count = ", win.ydata[-1] 
        ...
        if(win.dipoleTransferROIBackgnd.isChecked() and j == repetitions):
            print "Storing scattering background\n"
            win.adwPro2.Set_Par(78,1)
            win.adw.Set_Par(78,1)
            win.Background_IRScatt = np.mean(camObject.image, axis=0)
            imageio.imwrite("IRScatt.png", win.Background_IRScatt)
            np.save("IRScatt"+str(win.exposure_time.value())+"mus", win.Background_IRScatt)
            self.update_ccd_pic.emit(win.Background_IRScatt)
            ROI_profile = np.sum(win.Background_IRScatt,axis=win.roiProfilePlot.currentIndex())
            self.update_profile_plot.emit(ROI_profile, True, 0)
            
        # store last shadow, light and od picture into folder
        if (win.loadDipoleTrapAbsorpImag.isChecked() and i==2):
        ...
        if(win.motCurrentTTL.isChecked()):
            win.adw.Set_Par(3, win.adw.Get_Par(3) | 0b1000000000000) # switch on pwr supply
        if (hasattr(win,'ROIHistogramPlot')):
            delattr(win, 'ROIHistogramPlot')
            
        self.finished.emit()
        
        #if(not(win.dipoleTransferROIBackgnd.isChecked())):
        #    win.writeAnalogTimingGraph([9,6,7,10,11,8,12,14,16,13], directory2 + "/analogtiminggraph.csv", "Time [ADwin unit]\t Cooler VCA [V]\t Repump VCA [V]\t Dipole power [V]\t RF driver power [V]\t Beat VCO [V]\t MOT current [V]\t Imag Mod [V]\t DAQ enable [V]\t Cam TTL")
...
class MainGUIThread(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.adwPro2.Set_Par(5, int(time_unit/time_unit2))
            print "Trigger interval: ", int(time_unit/time_unit2)
            print "ADwin Pro II, time unit: ", time_unit2  
        ...
        self.plotOptions.addItem("Area under curve")
        self.plotOptions.addItem("Maximum count in ROI") 
        ...
        self.dict = { self.checkboxes.itemText(4): self.fiberLaserMod, self.checkboxes.itemText(3):self.optTrig, self.checkboxes.itemText(0) : self.coolerTTL, self.checkboxes.itemText(1):self.RFdriverTTL, self.analogOuts.itemText(0) : 1, self.analogOuts.itemText(1) : 4, self.checkboxes.itemText(2):self.motCurrentTTL, self.analogOuts.itemText(2):3}
        ...
        self.fitROIProfile = False
        self.doFit.stateChanged.connect(lambda: setattr(self, 'fitROIProfile', self.doFit.isChecked()))
        ...
        self.averageSample.setOpts(value = 10, step = 1, int = True)
        ...
        '''
        # step 2: create a QThread object (each for pixelfly qe and USB)
        ...
        # step 5: connects signals and slots
        ...
        self.tofgating.setParams(self.adwPro2,2)
        self.optTrig.setParams(self.adw, 23)
        self.camera2TTL.setParams(self.adwPro2, 0)
        self.daqEnable.setParams(self.adw, 29)
        self.femtoShutter.setParams(self.adw, 30)
        self.PASync.setParams(self.adwPro2, 10)
        self.IRSync.setParams(self.adwPro2, 11)
        self.PAbeamShutter.setParams(self.adwPro2, 3)
        self.PAfiberbeam.setParams(self.adwPro2,6)

        self.rabiAOMAux.setParams(self.adwPro2, 7)
        self.rabiAOMMain.setParams(self.adwPro2, 8)
        self.rabiShutter.setParams(self.adwPro2, 9)        


        # initialize AnalogOuts (derived from pyqtgraphs SpinBox)
        self.vcaCooler.setParams(self.adw, 1, 0, 0.69, 2, 0.01) 
        self.vcaRepumper.setParams(self.adw, 2, 0, 3.5, 2, 0.01)
        self.vcoBeatOffset.setParams(self.adw, 3, 0.0, 10.0, 2, 0.01)
        self.vcaCooler.setValue(0.69)
        self.vcaCooler.setAnalogOutput()
        self.vcaRepumper.setValue(3.5)        
        ...
        self.imagMod.setParams(self.adw, 7, 0.0, 2.0, 2, 0.01)
        self.imagMod.setValue(2.0)

        self.PAattenuator.setParams(self.adwPro2, 4, 0.0, 3.5, 2, 0.01)
        self.PAattenuator.setValue(0.0)
        self.PAattenuator.valueChanged.connect(self.PAattenuator.setAnalogOutput)
        self.PAattenuator.setAnalogOutput()

        self.dyeLasFreq.setParams(self.adwPro2, 6, -5.0, 5.0, 3, 0.01)
        self.dyeLasFreq.setValue(0)
        self.dyeLasFreq.valueChanged.connect(self.dyeLasFreq.setAnalogOutput)
        self.dyeLasFreq.valueChanged.connect(lambda: self.adwPro2.Set_FPar(67, self.dyeLasFreq.value()))
        self.dyeLasFreq.setAnalogOutput()

        
        self.PAfreq.setParams(self.adwPro2, 3, 0.0, 10, 2, 0.01)
        self.PAfreq.setValue(self.PAfreq.value())
        self.PAfreq.valueChanged.connect(self.PAfreq.setAnalogOutput)
        self.PAfreq.setAnalogOutput()
        ...
        self.scCavLockDyeSP.valueChanged.connect(lambda: self.adwPro2.Set_FPar(57, self.scCavLockDyeSP.value()))
        self.lockPA.clicked.connect(lambda: self.adwPro2.Set_Par(35, int(self.lockPA.isChecked())))
        self.lockDye.clicked.connect(lambda: self.adwPro2.Set_Par(47, int(self.lockDye.isChecked())))
        ...
        self.scanLaser.addItem('Diode laser')
        self.scanLaser.addItem('Dye laser')
        self.scanLaserDict = {0:self.scCavLockSP, 1:self.scCavLockDyeSP}
        ...
        self.PASync.stateChanged.connect(self.PASync.setDigitalOutput)
        self.IRSync.stateChanged.connect(self.IRSync.setDigitalOutput)
        ...
        self.optTrig.stateChanged.connect(self.optTrig.setDigitalOutput)
        ...
        self.tofgating.stateChanged.connect(self.tofgating.setDigitalOutput)
        self.PAbeamShutter.stateChanged.connect(self.PAbeamShutter.setDigitalOutput)
        self.PAfiberbeam.stateChanged.connect(self.PAfiberbeam.setDigitalOutput)

        self.rabiAOMAux.stateChanged.connect(self.rabiAOMAux.setDigitalOutput)
        self.rabiAOMMain.stateChanged.connect(self.rabiAOMMain.setDigitalOutput)
        self.rabiShutter.stateChanged.connect(self.rabiShutter.setDigitalOutput)
        
        self.timeStamp.stateChanged.connect(lambda: self.adwPro2.Set_Par(70, int(self.timeStamp.isChecked())))
        ...
        self.doPAFreqScan.clicked.connect(lambda: setattr(self, 'PAFreqInitArray', self.doPAFreqScan.isChecked()))
        ...
        self.meanROI2Cnt.clicked.connect(self.printMeanROI2Cnts)
        ...
        # for timed ionization
        self.ionizationTimer = QTimer(self)
        self.remainingTimeGUI = QTimer(self)
        self.ionizationTimer.setSingleShot(True)
        self.ionizationTimer.timeout.connect(lambda: self.daqEnable.setChecked(False))
        self.ionizationTimer.timeout.connect(self.remainingTimeGUI.stop)
        self.ionizationTimer.timeout.connect(lambda: self.remTime.display(self.ionizationTime.value()))
        self.ionizeLimitedTime.clicked.connect(self.startTimedIonization)
        self.remainingTimeGUI.timeout.connect(lambda: self.remTime.display(self.remTime.intValue()-1))
        

    def startTimedIonization(self):
        if(self.ionizationTimer.isActive()):
            self.ionizationTimer.stop()
            self.remainingTimeGUI.stop()
            self.daqEnable.setChecked(False)
            self.remTime.display(self.ionizationTime.value())
        else:
            self.daqEnable.setChecked(True)
            self.remainingTimeGUI.start(1000)
            self.remTime.display(self.ionizationTime.value())
            self.ionizationTimer.start(self.ionizationTime.value()*1000)        
    ...
    def printMeanROI2Cnts(self):
        meancnts = np.mean(self.roi2.getArrayRegion(camObject.pic,self.img))
        print "Mean counts in ROI 2: ", meancnts, "\n"
    ...
    def startReleaseRecapture(self):
        ...
        while t<= tmax and self.noUserInterrupt:
            ...
            if(self.roiProfilePlot.currentIndex()==0):
                afterExpansion = np.zeros((2,height+1))
            else:
                afterExpansion = np.zeros((2, width+1))
            sum_img = 0            
            ...
            xtmp = np.arange(afterExpansion[0].size) + 1
    ...
    def ionizeFromMOT(self):
        ...
        try:
            ...
            self.adwPro2.Set_Par(52, int(self.PASync.isChecked()))
            self.adwPro2.Set_Par(33, int(self.IRSync.isChecked()))
            # ramps
            # time for ramp
        ...
        try:
            ...
            self.adw.Set_Par(79, 1)
            self.adwPro2.Set_Par(79,1)
            # start case 6, but start it on ADWin Pro II first, because the sequence already runs and is triggering.
        ...
        while(self.noUserInterrupt):
            ...
            while ((trigger < 2) and self.noUserInterrupt):
                ...
                if camObject.waitForBuffer(0):
                    ...
                    ROI2 = self.roi2.getArrayRegion(camObject.image[0], self.img)
                    print "ROI 2 Mean Count = ", np.mean(ROI2), "\n"
    ...
    def startSequence(self):
        try:
            ...
            if (logging):
                ...
                logging = False
            # beam powers for pulsed cooler and repumper pulses
            self.adw.Set_FPar(42, self.coolerPulsed.value())
            self.adw.Set_FPar(43, self.repumpPulsed.value())
            # imaging detuning and beam powers
            self.adw.Set_FPar(51, self.imagDetuning.value())
            self.adw.Set_FPar(40, self.coolerImag.value())
            self.adw.Set_FPar(41, self.repumpImag.value())
            # imaging beam intensity
            ...
            # cooling time at final detuning
            self.adw.Set_Par(30, int(math.ceil(self.molasseCoolTime_2.value()/time_unit)))
            self.adwPro2.Set_Par(30, int(math.ceil(self.molasseCoolTime_2.value()/time_unit2)))
            # pushing beam time
            ...
            if (self.PAbeam.isChecked()):
                ...
                self.adw.Set_Par(29, 0)
                self.adwPro2.Set_Par(29, 0)
    ...
    def startOpticalTrapLoad(self):        
        ...
        self.worker.update_ccd_pic.connect(lambda x: self.img.setImage(x))
        self.worker.update_profile_plot.connect(lambda x, refresh, color_int: self.p3.plot(x,clear=refresh, pen=(color_int,3)))
        # step 6: start the thread
        self.thread.start()
        
        # final resets
        self.startSequence2.setEnabled(False)
        self.thread.finished.connect(lambda: self.startSequence2.setEnabled(True))
        self.thread.finished.connect(lambda: self.no_of_cycle.display(0))
    ...
    def imagFreqScanOpticalTrap(self):
        ...
        try:
            ...
            # for ionization of atoms out of dipole trap
            self.adw.Set_Par(44, int(self.openfsShut.isChecked()))
            # for data acquisition after magnetic field has been switched off
...
## this function might not properly work!!!
    ...
    def updatePlots(self):
        ...
        ROI_profile = selected.sum(axis=self.roiProfilePlot.currentIndex())
        self.p3.plot(ROI_profile,clear=True)
        if(self.doFit.isChecked()):
            if(self.fitROIProfile):
                self.fitROIProfile = False
                self.ROIProfileFit = np.zeros(np.shape(ROI_profile))
                xvalues = np.arange(0, np.shape(ROI_profile)[0])
                self.params = fitGaussianProfile(xvalues, ROI_profile, self.ROIProfileFit)
                print("Sigma from Gaussian Fit: ", np.sqrt(self.params[0][1]))
            if(hasattr(self, "ROIProfileFit")):
                self.p3.plot(self.ROIProfileFit, pen='y')
